...
import { SupplierOrder as Order } from '../config/models/supplierOrder';
...
const AutocompleteSupplierOrder: React.FC<AutocompleteOrderProps> = ({ onSelect, value }) => {
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = orders.filter((order) =>
      order.supplier.phoneNumber1.includes(event.target.value)||order.poID.includes(event.target.value.toUpperCase())
    );
      ...
      {isOpen && filteredOrders.length > 0 && (
        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
          {filteredOrders.map((order, index) => (
            ...
            >
              {order.poID} - {order.supplier?.phoneNumber1}
            </div>
          ))}
        </div>
      )}
...
export default AutocompleteSupplierOrder;